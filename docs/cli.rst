...
* :code:`--mode` lightspeedpy supports three modes of operation --- `sum`, `clip`, and `weight` --- which establish the way lightspeedpy infers the true source flux from the raw pixel output. `Sum` just adds the detected pixel output to estimate the source flux. `clip` first rounds to the nearest whole number of electrons, which can reduce noise in some cases. `weight` fits the source flux to the distribution of detected counts, and this is the most precise method.
...
* **progress bar colors**: Bias stacking has green and yellow progress bars in lightspeedpy. (Normal bias stacking is green, noise fitting is yellow.) The other plugins' operations are white. If you are running multiple analyses that use the same bias data, and the green and yellow bars take a long time to complete, consider using the `bias` plugin to make a processed bias file once. Provide that bias file in the :code:`--bias` argument to avoid the green and yellow progress bars during later steps.