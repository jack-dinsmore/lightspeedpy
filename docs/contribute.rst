...
3. (*Make the programming interface*) Edit `__init__.py` to define the plugin's python API. See the file for additional instructions. The API is imported using :code:`from lightspeedpy import PLUGIN_NAME`.
...
Specifically, the command line instructions  :code:`python -m lightspeedpy.PLUGIN_NAME` will run the code you wrote in step 2, and :code:`from lightspeedpy import PLUGIN_NAME` will import the API you defined in step 3. If you complete step 4 and run :code:`make html` from lightspeedpy's main `docs` directory, then your packages documentation will also be made locally. The HTML page is stored at `docs/_build/index.html`.
