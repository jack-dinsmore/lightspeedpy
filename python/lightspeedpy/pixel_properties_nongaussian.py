...
class PixelProperties:
    ...
    def save(self, filename, clobber):
        ...
        """
        h0 = fits.PrimaryHDU()
        h1 = fits.ImageHDU(data=self.bias, name="BIAS")
        h2 = fits.ImageHDU(data=self.widths, name="WIDTHS")
        if self.params is not None:
            h3 = fits.ImageHDU(data=self.params, name="PARAMS")
            h4 = fits.ImageHDU(data=self.single_mask.astype(np.uint8), name="MASK")
    ...
    def load(filename):
        with fits.open(filename) as hdul:
            ...
            if len(hdul) == 4:
                ...
                single_mask = np.ones_like(bias).astype(bool)
            elif len(hdul) == 5:
                params = np.array(hdul[3].data)
...
def fit_single(edges, counts):
    """
    Fit a single Gaussian to a list of histograms by minimizing the Cash statistic
    ...
    params = np.repeat(x0[:, None], counts.shape[1], axis=1).astype(float)
    normalization = n_counts / ADU_PER_ELECTRON

    # Gradient of the Cash statistic w.r.t. (mean, sigma, k, nu, norm); shape (5, p).
    def cash_gradient(p):
        outers = np.subtract.outer(centers, p[0])
        gradient = np.asarray(pearson_grad(outers, 0, p[1], p[2], p[3])) * normalization * p[4]
        model = pearson(outers, 0, p[1], p[2], p[3]) * normalization
        model = model + 1e-7 * n_counts
        gradient = np.concatenate([gradient, [model]])   # (5, e-1, p); last row = d model / d norm
        model = model * p[4]
        gradient = gradient * 2 * (1 - counts / model)
        return np.sum(gradient, axis=1)                  # (5, p)

    # Hessian: central-difference cash_gradient; per-parameter, per-pixel step.
    rel_step = 1e-4
    floor = np.array([1e-3, 1e-3, 1e-2, 1e-3, 1e-3])     # step floors for mean, sigma, k, nu, norm
    nparam = params.shape[0]

    def cash_hessian(p):
        H = np.empty((nparam, nparam, p.shape[1]))
        for i in range(nparam):
            step = rel_step * np.maximum(np.abs(p[i]), floor[i])   # (p,)
            pp = p.copy(); pm = p.copy()
            pp[i] = pp[i] + step
            pm[i] = pm[i] - step
            H[:, i, :] = (cash_gradient(pp) - cash_gradient(pm)) / (2 * step)
        return 0.5 * (H + H.transpose(1, 0, 2))          # symmetrise finite-difference noise

    for iteration in tqdm.tqdm(range(300), colour="yellow"):
        g = cash_gradient(params)                        # (5, p)
        H = cash_hessian(params)                         # (5, 5, p)

        # Newton step: solve H @ delta = g for every pixel, then params -= delta.
        Hp = np.moveaxis(H, 2, 0)                        # (p, 5, 5)
        gp = g.T                                         # (p, 5)
        diag = np.diagonal(Hp, axis1=1, axis2=2)         # (p, 5)
        Hp = Hp + 1e-6 * np.abs(diag)[:, :, None] * np.eye(nparam)[None]  # ridge: avoids singular solve
        delta = np.linalg.solve(Hp, gp[..., None])[..., 0].T             # (5, p)
...
def fit_triple(edges, counts):
    ...
    # Gradient of the Cash statistic w.r.t. all 9 params; shape (9, p).
    def cash_gradient(p):
        outers = np.subtract.outer(centers, p[0])
        m1 = pearson(outers, 0,        p[1], p[2], p[3]) * normalization
        m2 = pearson(outers, 0 + p[4], p[1], p[2], p[3]) * normalization
        m3 = pearson(outers, 0 - p[6], p[1], p[2], p[3]) * normalization
        g1 = np.asarray(pearson_grad(outers, 0,        p[1], p[2], p[3])) * normalization
        g2 = np.asarray(pearson_grad(outers, 0 + p[4], p[1], p[2], p[3])) * normalization
        g3 = np.asarray(pearson_grad(outers, 0 - p[6], p[1], p[2], p[3])) * normalization
        model = m1 * p[8] + m2 * p[5] + m3 * p[7]
        gradient = g1 * p[8] + g2 * p[5] + g3 * p[7]        # (4, e-1, p): mean, sigma, k, nu
        gradient = np.concatenate([gradient,
            [ g2[0] * p[5],      # d model / d delta_high
              m2,                # d model / d amp_high
             -g3[0] * p[7],      # d model / d delta_low
              m3,                # d model / d amp_low
              m1 ]])             # d model / d amp_mid   -> (9, e-1, p)
        model = model + 1e-7 * n_counts
        gradient = gradient * (1 - counts / model)
        return np.sum(gradient, axis=1)                     # (9, p)

    # Hessian: central-difference cash_gradient; per-parameter, per-pixel step.
    rel_step = 1e-4
    floor = np.array([1e-3, 1e-3, 1e-2, 1e-3, 1e-3, 1e-4, 1e-3, 1e-4, 1e-3])
    nparam = params.shape[0]

    def cash_hessian(p):
        H = np.empty((nparam, nparam, p.shape[1]))
        for i in range(nparam):
            step = rel_step * np.maximum(np.abs(p[i]), floor[i])
            pp = p.copy(); pm = p.copy()
            pp[i] = pp[i] + step
            pm[i] = pm[i] - step
            H[:, i, :] = (cash_gradient(pp) - cash_gradient(pm)) / (2 * step)
    ...
    for iteration in tqdm.tqdm(range(300), colour="yellow"):
        g = cash_gradient(params)                           # (9, p)
        H = cash_hessian(params)                            # (9, 9, p)

        # Newton step: solve H @ delta = g per pixel, then params -= delta.
        Hp = np.moveaxis(H, 2, 0)                           # (p, 9, 9)
        gp = g.T                                            # (p, 9)
        diag = np.diagonal(Hp, axis1=1, axis2=2)            # (p, 9)
        Hp = Hp + 1e-6 * np.abs(diag)[:, :, None] * np.eye(nparam)[None]  # ridge for invertibility
        delta = np.linalg.solve(Hp, gp[..., None])[..., 0].T             # (9, p)
